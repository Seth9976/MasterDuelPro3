...
namespace System.Diagnostics
{
	...
	{
		// Token: 0x06000899 RID: 2201 RVA: 0x0002DC4C File Offset: 0x0002BE4C
		internal static object GetRuntimeObject(string className, Type baseType, string initializeData)
		{
			...
			if (type == null)
			{
				throw new ConfigurationErrorsException(SR.GetString("Couldn't find type for class {0}.", new object[] { className }));
			}
			if (!baseType.IsAssignableFrom(type))
			{
				throw new ConfigurationErrorsException(SR.GetString("The specified type, '{0}' is not derived from the appropriate base type, '{1}'.", new object[] { className, baseType.FullName }));
			}
			Exception ex = null;
			try
			{
				if (string.IsNullOrEmpty(initializeData))
				{
					...
					if (constructor == null)
					{
						throw new ConfigurationErrorsException(SR.GetString("Couldn't find constructor for class {0}.", new object[] { className }));
					}
			...
			if (ex != null)
			{
				throw new ConfigurationErrorsException(SR.GetString("Could not create {0}.", new object[] { className }), ex);
			}
			throw new ConfigurationErrorsException(SR.GetString("Could not create {0}.", new object[] { className }));
		}

		// Token: 0x0600089A RID: 2202 RVA: 0x0002DED4 File Offset: 0x0002C0D4
		internal static bool IsOwnedTL(Type type)
		{
			return typeof(EventLogTraceListener) == type || TraceUtils.IsOwnedTextWriterTL(type);
		}

		// Token: 0x0600089B RID: 2203 RVA: 0x0002DEF0 File Offset: 0x0002C0F0
		...
		// Token: 0x0600089C RID: 2204 RVA: 0x0002DF28 File Offset: 0x0002C128
		...
		// Token: 0x0600089D RID: 2205 RVA: 0x0002DF48 File Offset: 0x0002C148
