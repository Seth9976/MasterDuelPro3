...
public class CinemachineRecomposer : CinemachineExtension
{
	...
	protected override void PostPipelineStageCallback(CinemachineVirtualCameraBase vcam, CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
	{
		if (stage == this.m_ApplyAfter)
		{
			...
			Quaternion qTilted = state.RawOrientation * Quaternion.AngleAxis(this.m_Tilt, Vector3.right);
			Quaternion qDesired = Quaternion.AngleAxis(this.m_Pan, state.ReferenceUp) * qTilted;
