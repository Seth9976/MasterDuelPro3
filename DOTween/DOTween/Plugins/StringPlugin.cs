...
namespace DG.Tweening.Plugins
{
	...
	{
		// Token: 0x06000341 RID: 833 RVA: 0x0000DBA8 File Offset: 0x0000BDA8
		...
		// Token: 0x06000342 RID: 834 RVA: 0x0000DBE8 File Offset: 0x0000BDE8
		...
		public override void SetChangeValue(TweenerCore<string, string, StringOptions> t)
		{
			...
			if (num > 3 && t.startValue.get_Chars(num - 1) == '>')
			{
				int i = num - 3;
				while (i > -1)
				{
					if (t.startValue.get_Chars(i) == '<')
					{
						if (t.startValue.get_Chars(i + 1) != '/')
						{
			...
			if (num2 > 3 && t.changeValue.get_Chars(num2 - 1) == '>')
			{
				int j = num2 - 3;
				while (j > -1)
				{
					if (t.changeValue.get_Chars(j) == '<')
					{
						if (t.changeValue.get_Chars(j + 1) != '/')
						{
		...
		// Token: 0x06000347 RID: 839 RVA: 0x0000DDC4 File Offset: 0x0000BFC4
		...
		// Token: 0x06000349 RID: 841 RVA: 0x0000E004 File Offset: 0x0000C204
		private StringBuilder Append(string value, int startIndex, int length, bool richTextEnabled)
		{
				...
				if (c == '<')
				{
					...
					if (match.Success)
					{
						if (!flag && !flag2)
						{
							...
							{
								if (value.get_Chars(i) == '<' && value.get_Chars(i + 1) != '/' && Array.IndexOf<char>(array, value.get_Chars(i + 2)) != -1)
								{
			...
			if (StringPlugin._OpenedTags.Count > 0 && num < length2 - 1)
			{
				while (StringPlugin._OpenedTags.Count > 0 && num < length2 - 1)
				{
					...
					if (match2.Value.get_Chars(2) == StringPlugin._OpenedTags[StringPlugin._OpenedTags.Count - 1])
					{
		...
		// Token: 0x0600034A RID: 842 RVA: 0x0000E278 File Offset: 0x0000C478
